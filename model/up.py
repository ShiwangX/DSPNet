...
class PixelShuffleUpsampleLayer(BaseModule):
...
class UpsampleModule(BaseModule):
    def __init__(self, input_chans=768, c_down_ratio=2, mode='mix_conv_first'):
        super().__init__()
        assert mode in ['normal', 'mix', 'mix_conv_first','PixelShuffle']
        upsample_layer = {
            ...
            'PixelShuffle': lambda : PixelShuffleUpsampleLayer(input_chans=input_chans, output_chans=input_chans // c_down_ratio),
        }[mode]()
