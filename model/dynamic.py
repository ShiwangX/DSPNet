...
class Up_Branch(nn.Module):
    def __init__(self, dims=[192, 48], img_size=512, is_extrapolation=True):
        ...
        self.groups = nn.ModuleList([
            nn.ModuleList([
                nn.ModuleDict({
                    "conv3d": MDSC(in_channels=dims[0]),
                    "attn": AA(
                        dim=dims[0],
                        input_resolution=input_res_group1,  
            ...
            nn.ModuleList([
                nn.ModuleDict({
                    "conv3d": MDSC(in_channels=dims[1]),
                    "attn": AA(
                        dim=dims[1],
                        input_resolution=input_res_group2,  
                        is_extrapolation=is_extrapolation
                    )
                }),
                nn.ModuleDict({
