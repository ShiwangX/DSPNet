...
class RegTrainer(Trainer):
    def setup(self):
        ...
        if 'fdst' in args.data_dir or 'ucsd' in args.data_dir or 'venice' in args.data_dir or 'dronecrowd' in args.data_dir:
            self.datasets = {x: [CrowdDensity(args.data_dir+'/'+x+'/'+file, args.is_gray, x, args.frame_number,
            ...
            self.datasets = {x: CrowdDensity(os.path.join(args.data_dir, x), args.is_gray, x, args.frame_number, args.crop_height,
            ...
            self.dataloaders = {x: DataLoader(self.datasets[x],
                                              ...
                                              pin_memory=(True if x == 'train' else False))
                                for x in ['train', 'val']}
        self.model = FusionNet(threshold=args.threshold,img_size=args.crop_height,is_extrapolation=True,is_train=True)
    ...
    def train_eopch(self):
        ...
        if 'fdst' in args.data_dir or 'ucsd' in args.data_dir or 'venice' in args.data_dir or 'dronecrowd' in args.data_dir:
            ...
            for file in file_list:
                for step, (imgs, targets, keypoints, mask,diffs) in enumerate(self.dataloaders['train'][file]):
                    ...
                    diffs = diffs.to(self.device).squeeze(0)

                    with torch.set_grad_enabled(True):
                        output = self.model(imgs,diffs)
                        output = output * mask


                        # MSE
                        loss = 1 / (1 * b0 * f0) * (self.criterion(output, targets))

                        self.optimizer.zero_grad()
                        loss.backward()
                        self.optimizer.step()

                        pre_count = torch.sum(output.view(f0, -1), dim=1).detach().cpu().numpy()
            ...
            for step, (imgs, targets, keypoints, mask, diffs) in enumerate(self.dataloaders['train']):
                b0, f0, c0, h0, w0 = imgs.shape
                assert b0 == 1
                imgs = imgs.to(self.device).squeeze(0)
                targets = targets.to(self.device).squeeze(0)
    ...
    def val_epoch(self):
        ...
        if 'fdst' in args.data_dir or 'ucsd' in args.data_dir or 'dronecrowd' in args.data_dir:
            ...
            for file in range(len(file_list)):
                epoch_res = []
                for imgs, keypoints,diffs in self.dataloaders['val'][file]:
            ...
            for file in range(len(file_list)):
                epoch_res = []
                for imgs, keypoints, masks,diffs in self.dataloaders['val'][file]:
                    b, f, c, h, w = imgs.shape
                    assert b == 1, 'the batch size should equal to 1 in validation mode'
                    imgs = imgs.to(self.device).squeeze(0)
            ...
            epoch_res = []
            for imgs, keypoints, diffs in self.dataloaders['val']:
                b, f, c, h, w = imgs.shape
                assert b == 1, 'the batch size should equal to 1 in validation mode'
                imgs = imgs.to(self.device).squeeze(0)
                diffs = diffs.to(self.device).squeeze(0)
